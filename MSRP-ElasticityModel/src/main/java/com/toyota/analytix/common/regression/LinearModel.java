...
public class LinearModel {
	...
	public LinearModel(double[] y, double[][] x, boolean intercept) {
		if (null != x && x.length > 0 && null != y && y.length > 0) {
			...
			try {
				...
				mrmDevLogger.error(exception.getMessage(), exception);
			}
	...
	private void setIntercept(boolean value) {
		if (null == olsMultipleLinearRegression) {
			throw new AnalytixRuntimeException("ols MultipleRegresstion is null");
		} else {
	...
	public double getIntercept() {
		double intercept;
		if (null == simpleRegression) {
			throw new AnalytixRuntimeException("simple regression is null");
		} else {
	...
	public List<Double> getCoefficients() {
		...
		if (null == olsMultipleLinearRegression) {
			throw new AnalytixRuntimeException("ols regression is null");
		} else {
